...
static void mqtt_pub_start_cb(void *arg, const char *topic, u32_t tot_len) {
  DEBUG_printf("mqtt_pub_start_cb called for topic: \"%s\"\n", topic);
...
err_t mqtt_test_publish(MQTT_CLIENT_T *state) {
  ...
  sprintf(buffer, "{\"message\":\"test dirtie node %d / %d \"}", state->received, state->counter);
  ...
  err = mqtt_publish(state->mqtt_client, "pico_w/test", buffer, 
...
err_t mqtt_test_connect(MQTT_CLIENT_T *state) {
  ...
  if (err != ERR_OK) {
    DEBUG_printf("mqtt_connect error %d\n", err);
  }
...
int connect_mqtt(char ssid[], char password[], char ip[]) {
  ...
  if (!succ) {
    DEBUG_printf("Invalid IP String %s, exiting", ip);
    return 1;
  }

  if (mqtt_run_test(state)) {
    DEBUG_printf("MQTT Test Failed, exiting");
    return 1;
  }
